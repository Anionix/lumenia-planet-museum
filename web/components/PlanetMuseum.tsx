    ...
    <main>
      <section classStyle={[museumStyles.hero]}>
        <div><p classStyle={[sphereStyles.eyebrow]}>LUMENIA</p>
          ...
          <a href="/atelier/william-morris/" classStyle={[museumStyles.entry]}>モリスの制作室を訪ねる →</a>
        </div>
      ...
      <section aria-label="制作室を選ぶ" classStyle={[museumStyles.exhibitions]}>
        ...
        <div classStyle={[portraitStyles.directory]}>{cssMuseumPeople.map(person => <a key={person.appearanceIdentifier} href={`/atelier/${person.slug}/`} classStyle={[portraitStyles.card]}>
      ...
      <section aria-label="展示を選ぶ" classStyle={[museumStyles.exhibitions]}>
        ...
        <div classStyle={[museumStyles.list]}>{exhibitions.map(exhibition => <article key={exhibition.slug}>
          ...
          <a href={`/atelier/${exhibition.slug}/`}
            classStyle={[museumStyles.entry]}>制作室を訪ねる →</a>
        </article>)}</div>
        <div classStyle={[museumStyles.archive]}><span>Lumeniaから続く、線・面・立体の展示。</span>
          <a href="/compare/css/" classStyle={[galleryStyles.evidenceLink]}>光のかたちを見る</a></div>
      </section>
    </main>
    <footer classStyle={[galleryStyles.footer]}><span>Lumenia Planet Museum</span><a href="/records/" classStyle={[galleryStyles.evidenceLink]}>出典と検算記録</a></footer>
  </div>;
}
