    ...
    <main>
      ...
      <section aria-label="展示を選ぶ" classStyle={[museumStyles.exhibitions]}>
        ...
        <div classStyle={[museumStyles.list]}>{exhibitions.map(exhibition => <article key={exhibition.slug}>
          ...
          <a href={exhibition.slug === 'william-morris' ? '/atelier/william-morris/' : `/planetarium/${exhibition.slug}/`}
            classStyle={[museumStyles.entry]}>{exhibition.slug === 'william-morris' ? '制作室を訪ねる →' : '作品と資料を見る →'}</a>
        </article>)}</div>
