...
export function createWorldGeometry(recipe) {
    ...
    if (!geometryCache.has(key)) {
      ...
      const factories = {
        ...
        annulus: () => {
          ...
          outline.absarc(0,0,parameters[0],0,Math.PI*2,false);
          hole.absarc(0,0,parameters[1],0,Math.PI*2,true);outline.holes.push(hole);
  ...
  for (const shape of recipe.shapes) {
    ...
    group.add(mesh);
    if (shape.collision !== false && shape.opacity === undefined) solids.push(mesh);
  }
