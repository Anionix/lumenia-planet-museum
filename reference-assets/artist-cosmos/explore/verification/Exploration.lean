...
-- UUIDv5: 6c91f02b-ae5a-5ef3-8d30-f9d7c4c1a0bd; the JavaScript frame delta's zero floor and cap.
def boundedFrameMilliseconds (delta : Int) : Nat :=
  if delta ≤ 0 then 0 else cap delta.toNat 50

-- UUIDv5: f32dd0f5-6f79-5c8c-9dce-7f8e9cfe28c2
theorem boundedFrameMillisecondsBound (delta : Int) :
    boundedFrameMilliseconds delta ≤ 50 := by
  unfold boundedFrameMilliseconds
  split
  · exact Nat.zero_le _
  · exact capBound _ _

-- UUIDv5: 5bd702ad-8c92-5fc0-91d1-ea6ed0d6d4da
theorem nonpositiveFrameMillisecondsAreZero (delta : Int) (nonpositive : delta ≤ 0) :
    boundedFrameMilliseconds delta = 0 := by
  unfold boundedFrameMilliseconds
  rw [if_pos nonpositive]
...
#print axioms boundedFrameMillisecondsBound
#print axioms nonpositiveFrameMillisecondsAreZero
